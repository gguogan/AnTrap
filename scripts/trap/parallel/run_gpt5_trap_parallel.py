...
"""Parallel eval suite runner for GPT (gpt-5.4 / gpt-5.4-mini) with trap injection.

Hits {endpoint}/v1/chat/completions with ``Authorization: Bearer sk-...``.
API key + endpoint are read (in order): --api_key/--endpoint flag >
shell OPENAI_API_KEY / OPENAI_BASE_URL > <repo>/.env.
"""
...
from android_world.agents import gpt5
...
_MODEL = flags.DEFINE_string('model', 'gpt-5.4-mini', 'GPT model id.')
_ENDPOINT = flags.DEFINE_string(
    'endpoint', '',
    'API host. If empty, falls back to OPENAI_BASE_URL env var (stripped of '
    'trailing /v1), then https://api.example.com.',
)
...
# GPT sampling / agent knobs
_TEMPERATURE = flags.DEFINE_float('temperature', None, 'Sampling temperature (None = provider default).')
_TOP_P = flags.DEFINE_float('top_p', None, 'Top-p sampling.')
...
def worker(
    ...
    trap_config,
    temperature, top_p, max_output_tokens, max_retry,
    ...
    try:
        ...
        llm_wrapper = infer_gpt5.Gpt5Wrapper(
        ...
        agent = gpt5.Gpt5Agent(
            env=env,
            llm=llm_wrapper,
            name=f'Gpt5_w{worker_id}',
...
def _main() -> None:
    ...
    api_key = _API_KEY.value or os.environ.get('OPENAI_API_KEY', '')
    if not api_key:
        raise RuntimeError(
            'No API key. Pass --api_key=<KEY>, set OPENAI_API_KEY, or write '
            'it into <repo>/.env.'
        )
    endpoint = (
        _ENDPOINT.value
        or os.environ.get('OPENAI_BASE_URL', '').rstrip('/').removesuffix('/v1')
        or 'https://api.example.com'
    )

    print(f'Parallel runner (GPT + Trap): {num_workers} workers')
    ...
    for i in range(num_workers):
        t = threading.Thread(
            ...
            args=(i, console_ports[i], grpc_ports[i], task_queue, result_queue, suite, output_lock,
                  ...
                  _TEMPERATURE.value, _TOP_P.value,
