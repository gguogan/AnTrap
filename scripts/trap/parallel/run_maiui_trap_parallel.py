...
"""Parallel eval suite runner for MAI-UI with trap injection."""
...
_MODEL = flags.DEFINE_string('model', 'MAI-UI-8B', 'Model name.')
...
# Sampling parameters
_TEMPERATURE = flags.DEFINE_float('temperature', None, 'Sampling temperature.')
_TOP_P = flags.DEFINE_float('top_p', None, 'Top-p sampling.')
_TOP_K = flags.DEFINE_integer('top_k', None, 'Top-k sampling.')
_MAX_TOKENS = flags.DEFINE_integer('max_tokens', None, 'Max output tokens.')
_PRESENCE_PENALTY = flags.DEFINE_float('presence_penalty', None, 'Presence penalty.')
...
def _main() -> None:
    ...
    print(f'Parallel runner (MAI-UI + Trap): {num_workers} workers')
