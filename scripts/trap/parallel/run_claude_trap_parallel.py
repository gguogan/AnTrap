...
"""Parallel eval suite runner for Claude Sonnet 4.6 with trap injection."""
...
_ENDPOINT = flags.DEFINE_string(
    'endpoint', '',
    'Claude API host. If empty, falls back to CLAUDE_ENDPOINT / '
    'ANTHROPIC_BASE_URL env var, then https://api.example.com.',
)
...
_MAX_HISTORY_IMAGES = flags.DEFINE_integer('max_history_images', 3, 'Max past screenshots.')
...
def worker(
    ...
    try:
        ...
        llm_wrapper = infer_claude.ClaudeRestWrapper(
        ...
        if trap_config.category != 'none':
            trap_controller = TrapController(
                trap_config, env=env, vllm_wrapper=llm_wrapper,
            )
...
def _main() -> None:
    ...
    print(f'Parallel runner (Claude Sonnet + Trap): {num_workers} workers')
    ...
    for i in range(num_workers):
        t = threading.Thread(
            ...
            args=(i, console_ports[i], grpc_ports[i], task_queue, result_queue, suite, output_lock,
                  ...
                  traj_output_path, _MAX_HISTORY_IMAGES.value, trap_config,
