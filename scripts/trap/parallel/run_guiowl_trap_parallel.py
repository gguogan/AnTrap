...
"""Parallel eval suite runner for GUI-Owl (1.0) with trap injection.

Based on run_gui_owl_parallel.py, adds:
...
_MODEL = flags.DEFINE_string('model', 'GUI-Owl-7B', 'Model name served by vllm.')
...
# ---------------------------------------------------------------------------
# Worker thread — GUI-Owl 1.0 with trap support
# ---------------------------------------------------------------------------
def worker(
...
def _main() -> None:
    ...
    print(f'Parallel runner (GUI-Owl 1.0 + Trap): {num_workers} workers')
