...
"""Parallel eval suite runner for UI-TARS with trap injection.

Based on run_uitars_parallel.py, adds:
- --trap_category / --trap_type / --trap_probability flags
- TrapController creation per worker
- Trap log saved alongside trajectory
"""
...
# ---------------------------------------------------------------------------
# Thread-aware stream that can capture per-thread output into buffers.
# ---------------------------------------------------------------------------
...
_MODEL = flags.DEFINE_string('model', 'UI-TARS-1.5-7B', 'Model name served by vllm.')
...
_TASK_RANDOM_SEED = flags.DEFINE_integer('task_random_seed', 30, 'Random seed for task randomness.')
_TASKS = flags.DEFINE_list('tasks', None, 'List of specific tasks to run. If None, run all tasks.')
_N_TASK_COMBINATIONS = flags.DEFINE_integer('n_task_combinations', 1, 'Number of task instances per template.')
_CHECKPOINT_DIR = flags.DEFINE_string('checkpoint_dir', '', 'Directory to save/resume checkpoints.')
_OUTPUT_PATH = flags.DEFINE_string('output_path', os.path.expanduser('~/android_world/runs'), 'Path to save results.')
_TRAJ_OUTPUT_PATH = flags.DEFINE_string('traj_output_path', '', 'Path to save trajectory screenshots.')
_API_KEY = flags.DEFINE_string('api_key', 'EMPTY', 'API key for the vllm server.')
_BASE_URL = flags.DEFINE_string('base_url', 'http://localhost:9001/v1', 'Base URL of the vllm server.')
_FIXED_TASK_SEED = flags.DEFINE_boolean('fixed_task_seed', False, 'Same task seed for multiple combinations.')
_MAX_HISTORY_IMAGES = flags.DEFINE_integer('max_history_images', 4, 'Maximum number of past screenshots in the prompt.')
_NUM_WORKERS = flags.DEFINE_integer('num_workers', 4, 'Number of parallel emulator workers.')
...
_RESUME_LOG = flags.DEFINE_string('resume_log', '', 'Path to a previous log file to resume from.')

# Trap flags
_TRAP_CATEGORY = flags.DEFINE_string(
    'trap_category', 'none',
    'Trap category: none, s_layer, t_layer, a_layer, overall',
)
_TRAP_TYPE = flags.DEFINE_string(
    'trap_type', 'none',
    'Trap sub-type within category (e.g., visual_obscuration, grounding_error).',
)
_TRAP_PROBABILITY = flags.DEFINE_float(
    'trap_probability', 0.3,
    'Per-step trigger probability for traps.',
)
_TRAP_SEED = flags.DEFINE_integer(
    'trap_seed', 42,
    'RNG seed for trap triggering (reproducibility).',
)
_MAX_TRAPS = flags.DEFINE_integer(
    'max_traps', 0,
    'Max trap events per episode (0 = unlimited). At least 1 is always guaranteed.',
)
_TRAP_PARAMS = flags.DEFINE_string(
    'trap_params', '{}',
    'JSON string of extra trap parameters (overrides TrapConfig defaults).',
)
...
def _parse_resume_log(log_path):
    ...
    if not traj_output_path:
        ...
        traj_output_path = os.path.join(
            os.path.dirname(os.path.dirname(log_path)), 'trajectory', traj_name
        )
...
# ---------------------------------------------------------------------------
# Build TrapConfig from flags
# ---------------------------------------------------------------------------
def _build_trap_config():
    extra = json.loads(_TRAP_PARAMS.value) if _TRAP_PARAMS.value else {}
    config = TrapConfig(
        category=_TRAP_CATEGORY.value,
        trap_type=_TRAP_TYPE.value,
        trigger_probability=_TRAP_PROBABILITY.value,
        seed=_TRAP_SEED.value,
        max_traps=_MAX_TRAPS.value,
        **extra,
    )
    return config
...
# ---------------------------------------------------------------------------
# Worker thread — UI-TARS with trap support
# ---------------------------------------------------------------------------
def worker(
    ...
    max_history_images, trap_config,
    spare_pool=None,
):
    tag = f'[W{worker_id}]'
    env = None
    ...
    try:
        ...
        # Clean up any lingering trap overlay popups
        ...
        # Create trap controller (pass vllm for LLM-generated popup content)
        trap_controller = None
        if trap_config.category != 'none':
            trap_controller = TrapController(trap_config, env=env,
                                             vllm_wrapper=vllm_wrapper)
            # Clean up any lingering trap overlay popups
            trap_controller._dismiss_trap_overlay()

        agent = ui_tars.UITarsAgent(
            env=env,
            vllm=vllm_wrapper,
            name=f'UITARS_w{worker_id}',
        ...
        agent.tmp_prefix = f'w{worker_id}_'  # Avoid /tmp screenshot collisions
        agent.get_task_name(suite)
        agent.transition_pause = None
        agent.trap_controller = trap_controller  # Inject trap controller
        ...
        def run_episode_fn(task):
            # Reset trap controller for each episode
            if trap_controller:
        ...
        while True:
            ...
            max_retries = 10  # Max retries to ensure trap triggers
            episode = None
            task_trap_log = []
            attempts = 1
            captured_out = ''
            ...
            for attempt in range(max_retries):
                # Create temp directory for this attempt's trajectory
                temp_traj = tempfile.mkdtemp(prefix=f'trap_retry_{attempt}_')
                original_output_path = agent.output_path
                agent.output_path = temp_traj  # Agent writes to temp dir

                _cap_stdout.start_capture()
                _cap_stderr.start_capture()
                try:
                    # Reset agent internal state for each attempt
                    agent.reset(go_home=False)  # Don't go home here, we do it below
                    task_instance.initialized = False  # Allow re-initialization on retry

                    # Reset trap controller before each attempt
                    if trap_controller:
                        trap_controller.reset()
                    ...
                    # Run the task (includes task.initialize_task)
                    ...
                    # Capture output before checking trap
                    ...
                    if trap_controller is None or trap_controller._traps_fired > 0:
                        # Success: move contents of temp traj into worker_traj
                        if worker_traj:
                            ...
                            shutil.rmtree(temp_traj)  # No traj needed
                        attempts = attempt + 1
                        break  # Done

                    # No trap triggered: discard temp traj and retry
                    shutil.rmtree(temp_traj)
                    _print_direct(f'{tag} No trap triggered (attempt {attempt+1}/{max_retries}), retrying {instance_name}...')

                except Exception:
                    # Error: capture output and discard temp traj
                    captured_out = _cap_stdout.stop_capture()
                    captured_err = _cap_stderr.stop_capture()
            ...
            if trap_controller:
                ...
                # Save trap log
                if worker_traj:
                    trap_log_path = os.path.join(
                        worker_traj, f'trap_log_{instance_name.replace("/", "_")}.jsonl'
                    )
                    trap_controller.save_trap_log(trap_log_path)

            # Build task_log from captured output
            task_log = ''
            ...
            # Save attempts in episode for statistics
            if episode:
...
def _main() -> None:
    ...
    print(f'Parallel runner (UI-TARS + Trap): {num_workers} workers')
    ...
    print(f'Spare emulators: console={spare_console}, grpc={spare_grpc}')
    ...
    # --- Determine checkpoint_dir and traj_output_path ---
    ...
    num_preloaded = len(preloaded_episodes)
    ...
    print(f'Tasks remaining: {num_remaining}, already done: {num_preloaded}')
    print(f'Checkpoint dir: {checkpoint_dir}')
    print(f'Trajectory dir: {traj_output_path}')
    ...
    for i in range(num_workers):
        t = threading.Thread(
            target=worker,
            args=(
                i, console_ports[i], grpc_ports[i],
                task_queue, result_queue, suite, output_lock,
                _MODEL.value, _API_KEY.value, _BASE_URL.value,
                _ADB_PATH.value, _EMULATOR_SETUP.value,
                traj_output_path, _MAX_HISTORY_IMAGES.value,
                trap_config,
                spare_pool,
            ),
    ...
    # Main loop: collect results, print grouped log + summary
    ...
    while completed < num_remaining:
        ...
        episode_meta['attempts'] = attempts  # Save attempts for statistics
        ...
        if err is not None:
            status = 'ERROR'
        elif sr is not None and sr > 0.5:
            status = 'OK'
        else:
            status = 'FAIL'

        # Build trap summary line
        trap_summary = ''
        if task_trap_log:
    ...
    if episodes_metadata:
        ...
        for ep in episodes_metadata:
            attempts = ep.get('attempts', 1)
            retry_counts[attempts] = retry_counts.get(attempts, 0) + 1
        print('\nRetry Statistics:')
        for attempts, count in sorted(retry_counts.items()):
            print(f'  {attempts} attempt(s): {count} tasks')
        total_attempts = sum(k * v for k, v in retry_counts.items())
        total_tasks = sum(retry_counts.values())
        if total_tasks > 0:
