...
"""Parallel eval suite runner for GUI-Owl-1.5 agent.

Output is grouped by task: each task's full log (model responses, steps,
errors) is buffered during execution and printed as one continuous block
when the task finishes, followed by the summary table.
"""
...
# Threads not in capture mode write directly to the real stream.
# ---------------------------------------------------------------------------
class _CapturingStream:
    def __init__(self, real_stream):
        self._real = real_stream
        self._buffers = {}           # thread_ident -> StringIO
...
def _patch_logging_handlers():
    """Patch logging handlers to write through our capturing streams."""
...
_MODEL = flags.DEFINE_string(
    'model', 'GUI-Owl-1.5-8B-Think', 'Model name served by vllm.',
)
...
_TASK_RANDOM_SEED = flags.DEFINE_integer(
    'task_random_seed', 30,
    'Random seed for task randomness. Default 30 aligns with mobileagent_official.',
)
...
_N_TASK_COMBINATIONS = flags.DEFINE_integer(
    ...
    'Number of task instances per template.',
)
_CHECKPOINT_DIR = flags.DEFINE_string(
    'checkpoint_dir', '', 'Directory to save/resume checkpoints.',
)
_OUTPUT_PATH = flags.DEFINE_string(
    'output_path', os.path.expanduser('~/android_world/runs'),
    'Path to save results.',
)
_TRAJ_OUTPUT_PATH = flags.DEFINE_string(
    'traj_output_path', '', 'Path to save trajectory screenshots.',
)
_API_KEY = flags.DEFINE_string(
    'api_key', 'EMPTY', 'API key for the vllm server.',
)
...
_MAX_HISTORY_IMAGES = flags.DEFINE_integer(
    ...
    'Maximum number of past screenshots in the prompt.',
)
_NUM_WORKERS = flags.DEFINE_integer(
    'num_workers', 4, 'Number of parallel emulator workers.',
)
_CONSOLE_PORTS = flags.DEFINE_string(
    'console_ports', '5554,5556,5558,5560',
    'Comma-separated console ports for each emulator.',
)
_GRPC_PORTS = flags.DEFINE_string(
    'grpc_ports', '8554,8556,8558,8560',
    'Comma-separated gRPC ports for each emulator.',
)
_RESUME_LOG = flags.DEFINE_string(
    'resume_log', '',
    'Path to a previous log file to resume from. Auto-extracts checkpoint_dir '
    'and traj_output_path. Failed (NaN) and unfinished tasks will be rerun.',
)
...
# ---------------------------------------------------------------------------
# Resume: parse a previous log file to extract checkpoint_dir & traj path
# ---------------------------------------------------------------------------
def _parse_resume_log(log_path):
    """Extract checkpoint_dir and traj_output_path from a previous log file.

    Returns (checkpoint_dir, traj_output_path).
    """
    checkpoint_dir = None
    ...
    # Fallback: derive traj from log filename if not found in log
    # log/log_XXX.log -> trajectory/traj_XXX
...
def _dismiss_trap_overlay(env):
    """Dismiss any lingering trap popup overlay."""
...
def worker(
    ...
    def _print_direct(msg):
        """Write directly to real stdout (not captured)."""
        with output_lock:
        ...
        for attempt in range(1, 4):
            try:
                ...
                if attempt < 3:
                    ...
                    # Try spare emulator before giving up
        ...
        while True:
            ...
            try:
                ...
                episode = {
                    ...
                    constants.EpisodeConstants.AGENT_NAME: f'GUIOwl15_w{worker_id}',
                }
            finally:
                captured_out = _cap_stdout.stop_capture()
                captured_err = _cap_stderr.stop_capture()

            # Merge captured stdout and stderr into one log block
            task_log = ''
...
def _main() -> None:
    ...
    print(f'Parallel runner: {num_workers} workers')
    ...
    if _RESUME_LOG.value:
        # Resume mode: extract from previous log
    ...
    for i in range(num_workers):
        t = threading.Thread(
            ...
            args=(
                ...
                traj_output_path, _MAX_HISTORY_IMAGES.value, spare_pool,
            ),
    ...
    while completed < num_remaining:
        ...
        # Print grouped: header → task log → summary table
        with output_lock:
            header = (
                f'\n{"="*80}\n'
