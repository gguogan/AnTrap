...
"""Parallel eval suite runner for Claude Sonnet 4.6.

Two API surfaces are supported via --api_style:

  --api_style=anthropic   POST {endpoint}/v1/messages   (api.anthropic.com)
  --api_style=openai      POST {endpoint}/v1/chat/completions  (generic OpenAI-compat proxy)

If --api_style is empty, the wrapper auto-detects from the endpoint host.
Endpoint + API key fall back to .env (CLAUDE_ENDPOINT / CLAUDE_API_KEY,
...
from android_world.agents import claude_sonnet
...
_MODEL = flags.DEFINE_string('model', 'claude-sonnet-4-6', 'Claude model id.')
_ENDPOINT = flags.DEFINE_string(
    'endpoint', '',
    'API host. If empty, falls back to CLAUDE_ENDPOINT / ANTHROPIC_BASE_URL '
    'env var, then https://api.example.com.',
)
_API_KEY = flags.DEFINE_string(
    'api_key', '',
    'API key. If empty, falls back to CLAUDE_API_KEY / ANTHROPIC_API_KEY env var.',
)
_API_STYLE = flags.DEFINE_string(
    'api_style', '',
    'Which surface to call: "anthropic" (/v1/messages) or "openai" '
    '(/v1/chat/completions). Empty = auto-detect from endpoint host.',
)
_ANTHROPIC_VERSION = flags.DEFINE_string(
    'anthropic_version', '2023-06-01',
    'anthropic-version header value (only used when api_style=anthropic).',
)
...
# Claude sampling / agent knobs
_TEMPERATURE = flags.DEFINE_float('temperature', 1.0, 'Sampling temperature.')
_MAX_TOKENS = flags.DEFINE_integer('max_tokens', 4096, 'Max output tokens per response.')
_THINKING_BUDGET_TOKENS = flags.DEFINE_integer(
    'thinking_budget_tokens', 0,
    'Extended-thinking budget (0 = disabled). Only honored on Anthropic API.',
)
...
_CAPACITY_RETRY_DELAY = flags.DEFINE_float(
    ...
    'Seconds to sleep between retries on 429/503/529 capacity errors.',
)
_CAPACITY_MAX_RETRY = flags.DEFINE_integer(
    'capacity_max_retry', 30,
    'Separate retry budget for 429/503/529 capacity errors.',
)
...
def worker(
    ...
    output_lock, model, api_key, endpoint, api_style, anthropic_version,
    adb_path, emulator_setup, traj_output_path,
    temperature, max_tokens, thinking_budget_tokens, max_retry,
    ...
    try:
        ...
        agent = claude_sonnet.ClaudeSonnetAgent(
            env=env,
            llm=llm,
            name=f'ClaudeSonnet_w{worker_id}',
        ...
        while True:
            ...
            try:
                ...
                episode = {
                    ...
                    constants.EpisodeConstants.AGENT_NAME: f'ClaudeSonnet_w{worker_id}',
                }
            finally:
...
def _main() -> None:
    ...
    if not api_key:
        raise RuntimeError(
            'No API key. Pass --api_key=<KEY>, set CLAUDE_API_KEY / '
            'ANTHROPIC_API_KEY, or write it into <repo>/.env.'
        )
    endpoint = (
        _ENDPOINT.value
        or os.environ.get('CLAUDE_ENDPOINT')
        or os.environ.get('ANTHROPIC_BASE_URL')
        or 'https://api.example.com'
    )

    print(f'Parallel runner (Claude Sonnet): {num_workers} workers')
    print(f'Model: {_MODEL.value}  Endpoint: {endpoint}  Style: {_API_STYLE.value or "auto"}')
    ...
    for i in range(num_workers):
        t = threading.Thread(
            ...
            args=(i, console_ports[i], grpc_ports[i], task_queue, result_queue, suite, output_lock,
                  _MODEL.value, api_key, endpoint, _API_STYLE.value, _ANTHROPIC_VERSION.value,
                  _ADB_PATH.value, _EMULATOR_SETUP.value,
                  traj_output_path,
                  _TEMPERATURE.value, _MAX_TOKENS.value, _THINKING_BUDGET_TOKENS.value,
                  _MAX_RETRY.value,
