...
"""Parallel eval suite runner for UI-TARS agent.

Output is grouped by task: each task's full log is buffered during execution
and printed as one continuous block when the task finishes, followed by the
summary table.
"""
...
_MODEL = flags.DEFINE_string(
    'model', 'UI-TARS-1.5-7B', 'Model name served by vllm.',
)
...
_TASK_RANDOM_SEED = flags.DEFINE_integer(
    'task_random_seed', 30,
    'Random seed for task randomness. Default 30 aligns with GUI-Owl.',
)
...
_RESUME_LOG = flags.DEFINE_string(
    'resume_log', '',
    'Path to a previous log file to resume from.',
)
...
# ---------------------------------------------------------------------------
# Worker thread — UI-TARS specific
# ---------------------------------------------------------------------------
def worker(
        ...
        while True:
            ...
            try:
                ...
                episode = {
                    ...
                    constants.EpisodeConstants.AGENT_NAME: f'UITARS_w{worker_id}',
                }
            finally:
...
def _main() -> None:
    ...
    print(f'Parallel runner (UI-TARS): {num_workers} workers')
    print(f'Console ports: {console_ports[:num_workers]}')
    print(f'gRPC ports:    {grpc_ports[:num_workers]}')
    print(f'Spare emulators: {list(zip(spare_console, spare_grpc)) if spare_console else "none"}')
