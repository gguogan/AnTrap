...
"""Parallel eval suite runner for Qwen3-VL agent."""
...
_MODEL = flags.DEFINE_string('model', 'Qwen3-VL-8B-Instruct', 'Model name served by vllm.')
...
_MAX_HISTORY_IMAGES = flags.DEFINE_integer('max_history_images', 4, 'Max past screenshots in prompt.')
# Sampling parameters (None = let vLLM use its default)
_TEMPERATURE = flags.DEFINE_float('temperature', None, 'Sampling temperature. Thinking models: 1.0, Instruct: 0.7.')
_TOP_P = flags.DEFINE_float('top_p', None, 'Top-p sampling. Thinking: 0.95, Instruct: 0.8.')
_TOP_K = flags.DEFINE_integer('top_k', None, 'Top-k sampling. Recommended: 20.')
_MAX_TOKENS = flags.DEFINE_integer('max_tokens', None, 'Max output tokens. Thinking models need 40960.')
_PRESENCE_PENALTY = flags.DEFINE_float('presence_penalty', None, 'Presence penalty. Thinking VL: 0.0, Instruct VL: 1.5.')
...
def worker(
    ...
    try:
        ...
        agent = qwen3vl.Qwen3VLAgent(
            env=env,
            vllm=vllm_wrapper,
            name=f'Qwen3VL_w{worker_id}',
        ...
        while True:
            ...
            try:
                ...
                episode = {
                    ...
                    constants.EpisodeConstants.AGENT_NAME: f'Qwen3VL_w{worker_id}',
                }
            finally:
...
def _main() -> None:
    ...
    print(f'Parallel runner (Qwen3-VL): {num_workers} workers')
    ...
    for i in range(num_workers):
        t = threading.Thread(
            ...
            args=(i, console_ports[i], grpc_ports[i], task_queue, result_queue, suite, output_lock,
                  ...
                  _TEMPERATURE.value, _TOP_P.value, _TOP_K.value, _MAX_TOKENS.value, _PRESENCE_PENALTY.value,
                  spare_pool),
