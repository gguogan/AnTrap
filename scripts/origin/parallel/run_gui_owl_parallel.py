...
"""Parallel eval suite runner for GUI-Owl (1.0) agent.
...
summary table.  Aligned with the serial runner run_ma3.py.
"""
...
# ---------------------------------------------------------------------------
# Flags — aligned with run_ma3.py defaults
# ---------------------------------------------------------------------------
...
_MODEL = flags.DEFINE_string(
    'model', 'GUI-Owl-7B', 'Model name served by vllm.',
)
...
# ---------------------------------------------------------------------------
# Worker thread — GUI-Owl 1.0 specific
# ---------------------------------------------------------------------------
def worker(
    ...
    try:
        ...
        # GUI-Owl 1.0: src_format='abs_resized', matching run_ma3.py line 174
        ...
        while True:
            ...
            try:
                ...
                episode = {
                    ...
                    constants.EpisodeConstants.AGENT_NAME: f'GUIOwl_w{worker_id}',
                }
            finally:
...
def _main() -> None:
    ...
    print(f'Parallel runner (GUI-Owl 1.0): {num_workers} workers')
