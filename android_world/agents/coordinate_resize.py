...
def smart_resize(height, width, factor=28, min_pixels=56 * 56, max_pixels=14 * 14 * 4 * 1280, max_long_side=8192):
    """Resize so the output image satisfies:
    1. Width and height are divisible by `factor`.
    2. Total pixel count is bounded by [min_pixels, max_pixels].
    3. Longest side is bounded by `max_long_side`.
    4. Aspect ratio is approximately preserved.
    """
...
def update_image_size_(image_ele: dict, min_tokens=1, max_tokens=12800, merge_base=2, patch_size=14):
    """Update size info on `image_ele` according to `min_tokens`, `max_tokens`.
    ...
    Returns:
        The updated `image_ele` with the following additional keys:
        dict:
            - image_ele["resized_height"]: int, height as seen by the model
            - image_ele["resized_width"]: int, width as seen by the model
            - image_ele["seq_len"]: int, sequence length consumed by this image
    """
