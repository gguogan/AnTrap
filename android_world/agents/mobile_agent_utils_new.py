...
def convert_mobile_agent_action_to_json_action(
    ...
    action_type_mapping = {
      ...
      "key": json_action.PRESS_KEYBOARD,
    }
    ...
    elif action_type == json_action.PRESS_KEYBOARD:
        # Convert model text (e.g. "volume_up", "clear", "enter") to KEYCODE_*
        raw = arguments.get('text', '').strip().upper().replace(' ', '_')
        keycode = f'KEYCODE_{raw}' if not raw.startswith('KEYCODE_') else raw
