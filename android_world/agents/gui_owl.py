...
from android_world.agents.coordinate_resize import update_image_size_, smart_resize
import traceback
...
def fetch_resized_image(screenshot_file):
    """Compute resized image and dimensions (original MobileAgent v3 behavior).

    Uses update_image_size_ which internally calls smart_resize with
    max_pixels=10035200.  The same max_pixels is used in image_to_base64
    (infer_ma3.py) and must also be set in the vLLM server's
    --mm-processor-kwargs to keep all three aligned.
    """
...
class GUIOwl(base_agent.EnvironmentInteractingAgent):
  ...
  def step(
    ...
    if self.output_path:
      ...
      # Find the key in self.task_name that is most similar to goal
      from difflib import SequenceMatcher
      def get_best_match(goal, task_name_dict):
        best_score = 0
        best_key = None
        for k in task_name_dict.keys():
          score = SequenceMatcher(None, goal, k).ratio()
          if score > best_score:
            best_score = score
            best_key = k
        return best_key
    ...
    if self.trap_controller and not np.array_equal(model_pixels, history_pixels):
        ...
        user_prompt_part['content'][-1] = {'image': tampered_file}
    ...
    try:
      if self.add_thought:
        # Parse thought (gracefully handle missing tags)
        if '</think>' in action_response:
          thought = action_response.split('</think>')[0].strip('<think>').strip('\n')
        elif '<thinking>' in action_response and '</thinking>' in action_response:
          thought = action_response.split('<thinking>')[1].split('</thinking>')[0].strip('\n')
        else:
          thought = None

        # Parse conclusion (gracefully handle missing tags)
        if '<conclusion>' in action_response and '</conclusion>' in action_response:
          summary = action_response.split('<conclusion>')[1].split('</conclusion>')[0].strip('\n')
        else:
          summary = None
      else:
        thought = None
      ...
      # Parse action (always required)
      if '{"name": "mobile_use"' in action_response:
        dummy_action = '{"name": "mobile_use"' + action_response.split('{"name": "mobile_use"')[1].split('}}\n')[0] + '}}'
      elif '<tool_call>' in action_response:
      ...
      if self.trap_controller:
          ...
          # If action type was remapped, replace in summary text
          if action.action_type != original_action_type and summary:
              _replace_map = {
              ...
              for old_type, old_words in _replace_map.items():
                  ...
                  for w in old_words:
                      if w in summary.lower():
                          summary = summary.replace(w, new_words[0])
                          summary = summary.replace(w.capitalize(), new_words[0].capitalize())
                          break
              ...
              # Grounding error: no need to change summary text, coords are invisible in summary
