...
def run_episode(
  ...
  # When traps are active: extend total steps to 1.6x, traps only in first 80%
  if trap_controller and trap_controller.config.category != 'none':
    trap_controller.max_steps = max_n_steps  # Original max_steps for trap window
    max_n_steps = int(max_n_steps * 1.15)    # Extended total steps
  ...
  for step_n in range(max_n_steps):
    if trap_controller:
      trap_controller.pre_step(agent.env, step_n)
