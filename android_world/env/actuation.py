...
def execute_adb_action(
  ...
  if action.action_type in ['click', 'double_tap', 'long_press']:
    ...
    text = action.text
    if text:
      # Resolve focus coordinates: prefer element center (when `index` given),
      # fall back to explicit (x, y). Skip the focusing long-press if neither
      # is available — previous code unconditionally passed `action.x/y`,
      # which are None when only `index` was set, producing adb errors like
      # `shell input swipe None None None None 1000`.
      focus_x = focus_y = None
      if action.index is not None:
        if 0 <= action.index < len(screen_elements):
          bbox = screen_elements[action.index].bbox_pixels
          if bbox is not None:
            cx, cy = bbox.center
            focus_x, focus_y = int(cx), int(cy)
      elif action.x is not None and action.y is not None:
        focus_x, focus_y = int(action.x), int(action.y)
      if focus_x is not None and focus_y is not None:
        adb_utils.long_press(focus_x, focus_y, env)
        time.sleep(1.0)
  ...
  elif action.action_type == json_action.STATUS:
    # Task is done, no action needed
    pass
  else:
