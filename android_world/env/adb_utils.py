...
_PATTERN_TO_ACTIVITY = immutabledict.immutabledict({
    ...
    'files|file manager|file explorer|file browser': 'com.google.android.documentsui/com.android.documentsui.files.FilesActivity',
...
def _iter_app_aliases():
  """Yield every alias (left side of '|') paired with its activity."""
  for pattern, activity in _PATTERN_TO_ACTIVITY.items():
    for alias in pattern.split('|'):
      yield alias.strip().lower(), activity


def fuzzy_match_activity(
    app_name: str, cutoff: float = 0.6
) -> Optional[str]:
  """Best-effort activity lookup for loosely-named apps.

  Models sometimes emit ``"Recorder"`` or ``"Google Calendar"`` when the
  AndroidWorld catalog calls them ``"audio recorder"`` and
  ``"google calendar"``. This function first tries a substring match against
  every alias in ``_PATTERN_TO_ACTIVITY`` (case-insensitive), then falls back
  to ``difflib``'s sequence matcher. Returns ``None`` if nothing is close
  enough (``cutoff`` = minimum similarity for the fuzzy stage).
  """
  if not app_name:
    return None
  normalized = app_name.strip().lower()
  aliases = list(_iter_app_aliases())

  # 1) Substring: catches "Recorder" -> "audio recorder",
  #    "Simple Gallery" -> "simple gallery pro", etc.
  for alias, activity in aliases:
    if alias and (alias in normalized or normalized in alias):
      return activity

  # 2) Token-overlap: any word of the input shares all its tokens with alias.
  input_tokens = set(re.findall(r'[a-z0-9]+', normalized))
  if input_tokens:
    best_activity = None
    best_overlap = 0
    for alias, activity in aliases:
      ...
      if not alias_tokens:
        continue
      overlap = len(input_tokens & alias_tokens)
      if overlap > best_overlap and overlap == len(input_tokens):
        best_overlap = overlap
        best_activity = activity
    if best_activity is not None:
      return best_activity

  # 3) difflib similarity fallback.
  alias_names = [a for a, _ in aliases]
  matches = difflib.get_close_matches(
      normalized, alias_names, n=1, cutoff=cutoff
  )
  if matches:
    for alias, activity in aliases:
      if alias == matches[0]:
...
def launch_app(
  ...
  if activity is None:
    # Regex-exact lookup failed (e.g. model emitted "Recorder" instead of
    # "audio recorder"). Try fuzzy matching before the package-name fallback.
    activity = fuzzy_match_activity(app_name)
    if activity is not None:
      logging.info('Fuzzy-matched %r -> %s', app_name, activity)
  if activity is None:
    #  If the app name is not in the mapping, assume it is a package name.
    if ' ' in app_name:
      # Real Android package names cannot contain spaces; passing one
      # straight to `monkey -p` blows up because adb shell tokenizes the
      # extra word as the count argument. This is almost always a model
      # output bug (e.g. "file manager" instead of the canonical "files");
      # surface a clean log line instead of crashing the episode.
      logging.warning(
          'launch_app: refusing to monkey-launch %r — package name '
          'contains spaces and did not match any alias.', app_name
      )
      return app_name
    response = issue_generic_request(
